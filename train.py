...
def train(fre_name: str) -> None:
    ...
    feature_list = feature_sel_dict[fre_name]
    ...
    print(INFO + "Dataset preprocessing done!" + END)
    ...
    for i in range(len(m_list)):
        ...
        m_list[i].cv_test(feature_list, 5, X_train, y_train)
        with codecs.open(
            ...
            file.writelines("The F1 Score result of {} :".format(m_list[i].name) + "\n")
            file.writelines("{}".format(m_list[i].f1) + "\n")
            file.writelines(
                "{:.9f}\t{:.9f}".format(np.average(m_list[i].f1), np.max(m_list[i].f1))
                + "\n"
            )
            file.writelines("\n")
            file.writelines("The Accuracy result of {} :".format(m_list[i].name) + "\n")
    ...
    plot_result(fre_name, group_name, m_list)
    for i in range(len(m_list)):
        with codecs.open(fre_name + "_metrics.txt", "a", "utf-8") as file:
            file.writelines("The F1 Score result of {} :".format(m_list[i].name) + "\n")
            file.writelines(
                "{:.9f}\t{:.9f}".format(np.average(m_list[i].f1), np.max(m_list[i].f1))
                + "\n"
